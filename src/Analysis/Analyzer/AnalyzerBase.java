...
public abstract class AnalyzerBase {
	...
	AnalyzerBase nextAnalyze;
	public AnalyzerBase next(AnalyzerBase nextAnayze) {
	...
	protected AnalysisResultModel AnalysisCarInfo(RoadInfoModel roadInfo, double BASE_SPEED_km_p_h, double TARGET_SPEED_km_p_h, double TARGET_Y ){
        ...
        // TTC 가중치 구하기
        if (result.TTC_T >= 0) {
        		setTTCWeights(result);
        }
        
        return result;
	}
	
	private void setTTCWeights(AnalysisResultModel result) {
		result.TTC_EXP = getTTC_EXP(result.TTC_T);
		result.TTC_Uniform_Kernel = getTTC_Uniform_Kernel(result.TTC_T, result.RAW_BETWEEN_DISTANCE);
		result.TTC_Triweight_Kernel = getTTC_Triweight_Kernel(result.TTC_T, result.RAW_BETWEEN_DISTANCE);
		result.TTC_Gaussian_Kernel = getTTC_Gaussian_Kernel(result.TTC_T, result.RAW_BETWEEN_DISTANCE);
	}
	...
	private double getTTC_Uniform_Kernel(double TTC_T, double DISTANCE) {
		double TTCw = getTTC_EXP(TTC_T);
		return 1.0/2.0 * TTCw;
	}
	
	private double getTTC_Triweight_Kernel(double TTC_T, double DISTANCE) {
		double TTCw = getTTC_EXP(TTC_T);
		double weight = 35/32*Math.pow((1-Math.pow(DISTANCE,2)),3);
		return weight * TTCw;
	}
	
	private double getTTC_Gaussian_Kernel(double TTC_T, double DISTANCE) {
		double TTCw = getTTC_EXP(TTC_T);
		double weight = (1/(Math.sqrt(2 * Math.PI)))*Math.pow(Math.E,(-1.0/2.0)*Math.pow(DISTANCE, 2));
		return weight * TTCw;
	}
